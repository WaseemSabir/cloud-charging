...
async function charge(account: string, charges: number): Promise<ChargeResult> {
    ...
    try {
        ...
        multi.get(balanceKey);
        // decrement balance by charges, if user is unauthorized 
        // (i.e. balance is less than charges), then this will be 
        // rolled back
        multi.decrBy(balanceKey, charges); 

        const [balanceStr, _] = await multi.exec();
